...
import { Task, Task_Progress_Status, USER_ROLE } from '@/app/config/gql-types';
		...
		>
			...
			>
				...
				<Text ff={'Nunito, sans-serif'} fz={21} fw={500}>
					{__task?.taskDetails?.taskName?.slice(0, 40)}
					{__task?.taskDetails?.taskName?.length > 40 && ' ...'}
				</Text>
					...
					<Menu position='left' withArrow>
						...
						<Menu.Dropdown>
							...
							<Menu.Item
								...
								disabled={user?.role === USER_ROLE.EMPLOYEE}
							>
								COMPLETED
							</Menu.Item>
							<Menu.Item
